...
void BlackHoleEnumerator::next()
{
    ...
    if (pr.result().count == 0)
        elem.averageWeight = 0;
    else
        elem.averageWeight = double(pr.result().sum) / pr.result().count;
...
int main()
{
    try {
        ...
        if (pr.found())
            cout << "The heaviest blackhole, which was observed nearer than 3 million light years at least once by every station: " << pr.optElem().blackHoleId << " (" << pr.opt() << " Suns).\n";
        else
            cout << "There are no black holes with the desired condition.\n";
